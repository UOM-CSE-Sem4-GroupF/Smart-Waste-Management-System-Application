...
import type { Bin, Route, Vehicle, Zone, WasteType } from '@/lib/types';
...
const ALL_STATUSES = ['normal', 'monitor', 'urgent', 'critical', 'offline'] as const;

type ForecastStatus = typeof ALL_STATUSES[number];
...
function fillColor(fill: number, offline: boolean): string {
  ...
  if (fill >= 75) return '#FB923C';
  if (fill >= 50) return '#FBBF24';
  return '#34D399';
}
...
function statusFromFill(bin: Bin): ForecastStatus {
...
export default function MapView({ bins, vehicles, routes, zones }: Props) {
  ...
  const binMarkersRef = useRef<Map<string, Marker>>(new Map());
    ...
    const timeoutId = window.setTimeout(() => {
      import('leaflet').then(L => {
        ...
        bins.forEach(bin => {
          ...
          if (activeFilters.zones.size > 0 && !activeFilters.zones.has(bin.zone)) return;
          if (activeFilters.types.size > 0 && !activeFilters.types.has(bin.type)) return;
          if (activeFilters.statuses.size > 0 && !activeFilters.statuses.has(status)) return;
          ...
          const existingMarker = existing.get(bin.id);
          if (existingMarker) {
            existingMarker.setIcon(icon);
            existingMarker.setPopupContent(binPopupHtml(bin));
            if (!map.hasLayer(existingMarker)) existingMarker.addTo(map);
          } else {
            const marker = L.marker([bin.lat, bin.lng], { icon });
            marker.bindPopup(binPopupHtml(bin));
            marker.addTo(map);
            existing.set(bin.id, marker);
          }
        });
        ...
        existing.forEach((marker, id) => {
          ...
          const status = statusFromFill(bin);
          const filtered =
            (activeFilters.zones.size > 0 && !activeFilters.zones.has(bin.zone)) ||
            (activeFilters.types.size > 0 && !activeFilters.types.has(bin.type)) ||
            (activeFilters.statuses.size > 0 && !activeFilters.statuses.has(status));

          if (filtered && map.hasLayer(marker)) marker.remove();
          if (!filtered && !map.hasLayer(marker)) marker.addTo(map);
        });
      });
    }, 50);
    ...
    import('leaflet').then(L => {
      ...
      vehicles.forEach(v => {
        ...
        if (marker) {
          ...
          marker.setIcon(icon);
          marker.setPopupContent(vehiclePopupHtml(v));
          if (show && !map.hasLayer(marker)) marker.addTo(map);
          if (!show && map.hasLayer(marker)) marker.remove();
        } else {
          const newMarker = L.marker([v.lat, v.lng], { icon });
          newMarker.bindPopup(vehiclePopupHtml(v));
          if (show) newMarker.addTo(map);
          existing.set(v.id, newMarker);
        }
      });
    });
  }, [vehicles, filters]);
  ...
  useEffect(() => {
    ...
    import('leaflet').then(L => {
      ...
      activeRoutes.forEach((route, idx) => {
        ...
        const existingLine = existing.get(route.id);
        if (existingLine) {
          existingLine.setLatLngs(latlngs);
          if (show && !map.hasLayer(existingLine)) existingLine.addTo(map);
          if (!show && map.hasLayer(existingLine)) existingLine.remove();
        } else {
  ...
  return (
    <div style={{ position: 'relative', height: '100%', borderRadius: 10, overflow: 'hidden', border: '1px solid var(--border)' }}>
      ...
      <button
        onClick={() => setFilters(f => ({ ...f, showZones: !f.showZones }))}
        style={{
          position: 'absolute', top: 12, right: 12, zIndex: 1000,
          background: filters.showZones ? 'var(--accent)' : 'rgba(15,22,36,0.92)',
          border: `1px solid ${filters.showZones ? 'var(--accent)' : 'var(--border)'}`,
          color: filters.showZones ? '#0F1624' : 'var(--text-secondary)',
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {ALL_TYPES.map(t => (
              <button
                key={t}
                style={activeBtn(filters.types.has(t))}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {zoneIds.map(zId => {
                ...
                return (
                  <button
                    key={zId}
                    style={activeBtn(filters.zones.has(zId), zone?.color ?? 'var(--accent)')}
