 ...
 */
function validateServiceHeader(req, reply) {
...
async function internalRoutes(app) {
    ...
    // =========================================================================
    app.post('/internal/clusters/:cluster_id/snapshot', async (req, reply) => {
        if (!validateServiceHeader(req, reply))
            return;
        ...
        try {
            ...
            const bins = clusterBins.map((bin) => {
                ...
                return {
                    ...
                    avg_kg_per_litre: weightCalculator_1.AVG_KG_PER_LITRE[bin.waste_category],
            ...
            logger.error({ cluster_id, error: error instanceof Error ? error.message : String(error) }, 'Failed to generate cluster snapshot');
    ...
    // =========================================================================
    app.post('/internal/clusters/:cluster_id/scan-nearby', async (req, reply) => {
        if (!validateServiceHeader(req, reply))
            return;
        const { cluster_id } = req.params;
        ...
        try {
            ...
            logger.info({ cluster_id, zone_id, found_clusters: clusters.length }, 'Nearby clusters scan completed');
            return { clusters };
        }
        catch (error) {
            logger.error({
    ...
    // =========================================================================
    app.post('/internal/bins/:bin_id/mark-collected', async (req, reply) => {
        if (!validateServiceHeader(req, reply))
            return;
        const { bin_id } = req.params;
        const { job_id, driver_id, collected_at, fill_level_at_collection, actual_weight_kg } = req.body;
        try {
            const bin = store_1.store.getBin(bin_id);
            if (!bin) {
                return reply.code(404).send({
            ...
            // Update bin state
            const updated = store_1.store.upsertBin({
                bin_id,
            ...
            await (0, dashboardPublisher_1.publishToDashboard)({
                version: '1.0',
            ...
            logger.info({ bin_id, job_id, driver_id, collected_at }, 'Bin marked as collected');
            return {
                success: true,
                bin_id,
                collected_at,
