        ...
        eachMessage: async ({ topic, message }) => {
            ...
            try {
                ...
                if (topic === 'waste.vehicle.location') {
                    await handleVehicleLocation(envelope);
                }
                else if (topic === 'waste.vehicle.deviation') {
                    await handleVehicleDeviation(envelope);
                }
            }
            catch (e) {
...
async function handleVehicleLocation(event) {
    const { payload } = event;
    const { vehicle_id, driver_id, lat, lng, speed_kmh, heading_degrees, accuracy_m } = payload;
    // Update vehicle position
    const vehicle = store_1.vehicles.get(vehicle_id);
    if (vehicle) {
        vehicle.lat = lat;
        vehicle.lng = lng;
    }
    // Find active job for this vehicle
    const activeJob = Array.from(store_1.activeJobs.values()).find(job => job.assigned_vehicle_id === vehicle_id && job.state === 'IN_PROGRESS');
    if (!activeJob) {
    ...
    const routePlan = Array.from(store_1.routePlans.values()).find(rp => rp.job_id === activeJob.job_id);
    const binRecords = Array.from(store_1.binCollectionRecords.values()).filter(br => br.job_id === activeJob.job_id);
    ...
    const vehicleInfo = store_1.vehicles.get(vehicle_id);
    ...
    for (const binRecord of binRecords) {
        if (binRecord.arrived_at || binRecord.collected_at || binRecord.skipped_at)
            continue;
    ...
    if (routePlan) {
        ...
        const completedClusters = new Set(binRecords.filter(br => br.collected_at).map(br => {
...
async function handleVehicleDeviation(event) {
    const { payload } = event;
    ...
    if (!driver)
        return;
    const message = `${store_1.vehicles.get(vehicle_id)?.name || vehicle_id} is ${deviation_metres}m off planned route`;
