...
async function collectionsRoutes(app) {
    // POST /api/v1/collections/:job_id/bins/:bin_id/collected
    app.post('/api/v1/collections/:job_id/bins/:bin_id/collected', async (req, reply) => {
        ...
        const binRecord = store_1.binCollectionRecords.get(binKey);
        if (!binRecord) {
        ...
        const allBinRecords = Array.from(store_1.binCollectionRecords.values()).filter(br => br.job_id === job_id);
    ...
    app.post('/api/v1/collections/:job_id/bins/:bin_id/skip', async (req, reply) => {
        const { job_id, bin_id } = req.params;
        ...
        return {
            ...
            job_progress: {
                ...
                cargo_weight_kg: allBinRecords
                    .filter(br => br.collected_at)
                    .reduce((sum, br) => sum + (br.actual_weight_kg || br.estimated_weight_kg), 0),
                cargo_limit_kg: store_1.vehicles.get(job.assigned_vehicle_id)?.max_cargo_kg || 0,
                job_complete: job.state === 'COMPLETED'
            }
        };
    });
    // GET /api/v1/jobs/:job_id/progress
    app.get('/api/v1/jobs/:job_id/progress', async (req, reply) => {
        ...
        const driver = store_1.drivers.get(job.assigned_driver_id);
        const vehicle = store_1.vehicles.get(job.assigned_vehicle_id);
        const routePlan = Array.from(store_1.routePlans.values()).find(rp => rp.job_id === job_id);
        const allBinRecords = Array.from(store_1.binCollectionRecords.values()).filter(br => br.job_id === job_id);
        const binsCollected = allBinRecords.filter(br => br.collected_at).length;
        const binsSkipped = allBinRecords.filter(br => br.skipped_at).length;
        const binsPending = allBinRecords.length - binsCollected - binsSkipped;
        ...
        const waypoints = routePlan?.waypoints.map((wp, index) => ({
            ...
            cluster_name: 'Mock Cluster', // Would look up from cluster data
            bins: wp.bins,
            status: 'pending', // Would determine based on progress
            arrived_at: null,
            completed_at: null
        })) || [];
        const response = {
